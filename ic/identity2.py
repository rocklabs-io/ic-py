...
import ecdsa, ed25519
import hashlib
from .der import to_der, from_der
...
class Identity:
    def __init__(self, privkey = "", type = "ed25519", anonymous = False):
        ...
        if type == 'secp256k1':
            ...
            self._der_pubkey = to_der(self._pubkey, self.key_type)
        elif type == 'ed25519':
            if len(privkey) > 0:
            ...
            self._privkey = self.sk.to_ascii(encoding='hex').decode()
            self._pubkey = self.vk.to_ascii(encoding='hex').decode()
            self._der_pubkey = to_der(self._pubkey, self.key_type)
        else:
            raise 'unsupported identity type'

    @staticmethod
    def from_pem(pem: str, key_type: str):
